...
- TiDB 的**联合索引只占用一个名额**，例如: table\_1 有 **`1个`唯一索引**, **`1个`联合索引** 。 例如计算大事务限制是`30万限制/(1+索引数量)` 计算方式为 `30W / (1+1+1) = 10W` **[事务限制计算参考资料](https://www.jianshu.com/p/7a2fd8c2af25?from=timeline "事务限制计算参考资料")**
