...
- **`server`** 协程会无限循环以从 **`chan *Request`** 接收请求，并且为了避免被长时间操作所堵塞，它将为**每一个请求启动一个协程**来做具体的工作。 [![](http://qiniu.dev-share.top/image/png/golang_c_s.png)](http://qiniu.dev-share.top/image/png/golang_c_s.png)
...
func step1(in chan int, out chan int) {
    ...
    // 1.1 阻塞通道，等待传入数据
    temp := <-in
    fmt.Println("step1 --->", temp)
    // 1.2 将数据传入下一个通道中，并且+1
    out <- temp + 1
}
func step2(in chan int, out chan int) {
    time.Sleep(2e9) // 模拟业务处理耗时[2秒]
    // 阻塞通道，等待传入数据
    temp := <-in
    fmt.Println("step2 --->", temp)
    // 将数据传入下一个通道中，并且+1
    out <- temp + 1
}
func step3(in chan int, out chan int) {
    time.Sleep(2e9) // 模拟业务处理耗时[2秒]
    // 阻塞通道，等待传入数据
    temp := <-in
    fmt.Println("step3 --->", temp)
    // 将数据传入下一个通道中，并且+1
    out <- temp + 1
}
...
func main() {
    ...
    for {
        // 2.4.1 等待通道的信号
        select {
        // 2.4.2 stop通道一但接收到信号，将表示程序可以结束运行了
        case s := <-stop:
            fmt.Println("停止 --->", s)
