---
title: "SeekDB 测试代码"
date: "2025-12-17"
categories: 
  - "databases"
---



# SeekDB 测试代码



#### .env

``` ini
# 配置远程SeekDB服务地址
SEEKDB_HOST=*.*.*.*
SEEKDB_PORT=2881
SEEKDB_USER=root
SEEKDB_PASSWD=youpasswd
```



#### main.py

``` python
# -*- coding: utf-8 -*-
import os
from dotenv import load_dotenv
from sqlalchemy import create_engine, text
from sqlalchemy.orm import sessionmaker

load_dotenv()

# ===============================
# SeekDB 连接信息
# ===============================
HOST = os.getenv("SEEKDB_HOST")
PORT = os.getenv("SEEKDB_PORT")
USER = os.getenv("SEEKDB_USER")
PASSWORD = os.getenv("SEEKDB_PASSWD")
DATABASE = "seekdb"

DATABASE_URL = (
    f"mysql+pymysql://{USER}:{PASSWORD}@{HOST}:{PORT}/{DATABASE}?charset=utf8mb4"
)

engine = create_engine(
    DATABASE_URL,
    echo=False,
    future=True,
)

Session = sessionmaker(bind=engine, autoflush=False, autocommit=False)
session = Session()


# -------------------------------------------------
# 1️⃣ 建表（单 FULLTEXT + 向量）
# -------------------------------------------------
def create_table():
    session.execute(text("DROP TABLE IF EXISTS t2"))

    sql = """
    CREATE TABLE t2 (
        id INT PRIMARY KEY,
        lang VARCHAR(4),
        doc VARCHAR(200),
        fulltext_doc TEXT,
        embedding VECTOR(3),

        FULLTEXT INDEX idx_ft_all (fulltext_doc)
            WITH PARSER ngram
            PARSER_PROPERTIES = (ngram_token_size = 2),

        VECTOR INDEX idx_vec_embedding (embedding)
            WITH (distance = L2, type = HNSW)
    );
    """
    session.execute(text(sql))
    session.commit()
    print("✅ 表 t2 创建完成（单 FULLTEXT + VECTOR）")


# -------------------------------------------------
# 2️⃣ 插入示例数据
# -------------------------------------------------
def insert_sample_data():
    data = [
        # ===== 中文 zh =====
        (1, 'zh', '苹果', '苹果是一种常见的水果。', '[1.0,1.0,1.0]'),
        (2, 'zh', '香蕉', '香蕉是一种热带水果。', '[1.1,1.0,0.9]'),
        (3, 'zh', '西瓜', '西瓜是夏季水果。', '[2.0,2.0,2.0]'),
        (4, 'zh', '咖啡', '咖啡是一种提神饮品。', '[0.5,0.6,0.4]'),
        (5, 'zh', '手机', '智能手机已经普及。', '[3.0,3.0,3.0]'),
        (6, 'zh', '耳机', '无线耳机很流行。', '[2.9,3.1,3.0]'),
        (7, 'zh', '手表', '智能手表支持健康监测。', '[1.5,1.4,1.6]'),
        (8, 'zh', '汽车', '电动汽车发展迅速。', '[5.0,5.1,4.9]'),
        (9, 'zh', '冰箱', '冰箱是家庭电器。', '[4.0,4.1,3.9]'),
        (10, 'zh', '游戏机', '游戏机很受欢迎。', '[3.5,3.4,3.6]'),

        # ===== 日文 ja =====
        (101, 'ja', 'りんご', 'りんごは人気の果物です。', '[1.0,1.0,1.1]'),
        (102, 'ja', 'バナナ', 'バナナは果物です。', '[1.1,1.0,0.9]'),
        (103, 'ja', 'スイカ', 'スイカは夏の果物。', '[2.0,2.0,2.1]'),
        (104, 'ja', 'コーヒー', 'コーヒーは飲み物です。', '[0.5,0.6,0.4]'),
        (105, 'ja', 'スマートフォン', 'スマートフォンは必需品。', '[3.0,3.0,3.1]'),
        (106, 'ja', 'ヘッドホン', 'ノイズキャンセリング搭載。', '[2.9,3.1,3.0]'),
        (107, 'ja', 'スマートウォッチ', '健康管理に役立つ。', '[1.5,1.6,1.4]'),
        (108, 'ja', '電気自動車', 'EVが普及している。', '[5.0,5.1,4.9]'),
        (109, 'ja', '冷蔵庫', '家庭用冷蔵庫。', '[4.0,4.1,3.9]'),
        (110, 'ja', 'ゲーム機', 'ゲーム機は人気。', '[3.5,3.4,3.6]'),

        # ===== 英文 en =====
        (201, 'en', 'Apple', 'Apple is a fruit.', '[1.0,1.0,1.0]'),
        (202, 'en', 'Banana', 'Bananas are fruits.', '[1.1,1.0,0.9]'),
        (203, 'en', 'Watermelon', 'Watermelon is popular.', '[2.0,2.0,2.0]'),
        (204, 'en', 'Coffee', 'Coffee is a beverage.', '[0.5,0.6,0.4]'),
        (205, 'en', 'Smartphone', 'Smartphones are essential.', '[3.0,3.0,3.0]'),
        (206, 'en', 'Headphones', 'Wireless headphones.', '[2.9,3.1,3.0]'),
        (207, 'en', 'Smartwatch', 'Tracks health data.', '[1.5,1.4,1.6]'),
        (208, 'en', 'Electric Car', 'Electric cars are popular.', '[5.0,5.1,4.9]'),
        (209, 'en', 'Refrigerator', 'Home appliance.', '[4.0,4.1,3.9]'),
        (210, 'en', 'Game Console', 'Popular worldwide.', '[3.5,3.4,3.6]'),

        # ===== 混合 mix =====
        (301, 'mix', 'Apple iPhone', 'Apple iPhone 是スマートフォン。', '[1.1,1.1,1.1]'),
        (302, 'mix', 'Apple Watch', 'Apple Watch 是 smartwatch。', '[0.9,1.0,0.8]'),
        (303, 'mix', '咖啡 Coffee コーヒー', '咖啡 Coffee コーヒー 是饮品。', '[0.5,0.6,0.4]'),
        (304, 'mix', 'Tesla Model 3', 'Tesla 是 electric car。', '[5.0,5.1,4.9]'),
        (305, 'mix', 'Nintendo Switch', '游戏机 game console。', '[3.0,3.1,2.9]')
    ]

    sql = """
    INSERT INTO t2 (id, lang, doc, fulltext_doc, embedding)
    VALUES (:id, :lang, :doc, :fulltext_doc, :embedding)
    """

    params = [
        dict(
            id=d[0],
            lang=d[1],
            doc=d[2],
            fulltext_doc=d[3],
            embedding=d[4],
        )
        for d in data
    ]

    session.execute(text(sql), params)
    session.commit()
    print(f"✅ 插入数据 {len(data)} 条")


# -------------------------------------------------
# 3️⃣ 全文检索
# -------------------------------------------------
def query_ft(keyword, prefer_lang=None):
    print(f"\n🔍 FULLTEXT 查询: {keyword}")

    sql = """
    SELECT id, lang, doc,
           MATCH(fulltext_doc) AGAINST(:kw) AS score
    FROM t2
    WHERE MATCH(fulltext_doc) AGAINST(:kw)
    """

    params = {"kw": keyword}

    if prefer_lang:
        sql += " ORDER BY (lang = :lang) DESC, score DESC"
        params["lang"] = prefer_lang
    else:
        sql += " ORDER BY score DESC"

    result = session.execute(text(sql), params)
    for row in result.fetchall():
        print(row)


# -------------------------------------------------
# 4️⃣ 向量检索
# -------------------------------------------------
def query_vector(vec, limit=5):
    print(f"\n🔍 向量检索 TOP {limit}")

    sql = f"""
    SELECT id, lang, doc
    FROM t2
    ORDER BY l2_distance(embedding, :vec)
    LIMIT {limit}
    """

    result = session.execute(text(sql), {"vec": vec})
    for row in result.fetchall():
        print(row)


# -------------------------------------------------
# 5️⃣ 主流程
# -------------------------------------------------
if __name__ == "__main__":
    create_table()
    insert_sample_data()

    query_ft("スマートフォン", prefer_lang="ja")
    query_ft("smartwatch", prefer_lang="en")
    query_ft("コーヒー")
    query_ft("fruit")
    query_ft("水果")

    query_vector("[0.9,1.0,0.9]")

    session.close()
    engine.dispose()
    print("\n✅ 完成")

```

