...
1. **书店验证授权书**：
    - 小明来到书店A的借书柜台，将图书馆发放的授权书交给书店A的系统。书店A需要确认这份授权书的合法性，于是它联系了图书馆，确认该授权书是为小明生成的，并且没有被使用过。

2. **获取借书凭证**：
    - 确认无误后，图书馆颁发了一张临时借书凭证（类似于**访问令牌**），授权小明在书店A借书。这张临时凭证是小明借书的唯一凭证，它会在一段时间后过期。
...
- 图书馆相当于授权服务器，负责颁发借书卡、授权书和临时借书凭证。
- 书店A是资源服务器，负责提供图书并验证凭证。
- 授权书相当于授权码，是临时的，用户必须使用它换取长期有效的借书凭证（访问令牌）。
- 借书凭证由图书馆颁发，书店A需要向图书馆核实凭证的有效性。
