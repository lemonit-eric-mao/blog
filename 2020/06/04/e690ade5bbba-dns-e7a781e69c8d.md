...
date: '2020-06-04T03:45:44+00:00'
status: publish
permalink: /2020/06/04/%e6%90%ad%e5%bb%ba-dns-%e7%a7%81%e6%9c%8d
author: 毛巳煜
excerpt: ''
type: post
id: 5343
...
```ruby
cat > /home/deploy/coredns/docker-compose.yaml 
```
...
```ruby
cat > /home/deploy/coredns/config/Corefile 
```
...
```ruby
cat > /home/deploy/coredns/config/hosts 
```
...
**为什么要用 DNS私服？**  
 因为 每次都使用修改 `/etc/hosts` 文件做域名解析，太麻烦， 一台机器还好，如果有 几百台机器，一个一个的配置，这个工作量可想而知，那么为了解决这个问题，所以才使用 `DNS私服`做统一域名映射配置。
...
```conf
cat > /home/deploy/dns/config/dnsmasq.conf  域名解析成指定的 <ipaddr> 地址。
...
```ruby
cat > /home/deploy/dns/docker-compose.yml 
```
...
#add-mac[=base64|text]
#add-cpe-id=<string>
#add-subnet[[=[<ipv4 address="">/]<ipv4 length="" prefix="">][,[<ipv6 address="">/]<ipv6 length="" prefix="">]]
##############################################################################
...
#servers-file=<file>
</file></file-extension></directory></end></start></mtu></connections></interface></directory></interface></interface></router></ra-interval></interface></address></address></domain></alias></alias></interface></path></path></uid></enterprise-id></network-id></network-id></client></server></timeout></prompt></tag></timeout></prompt></server_name></server></bootservicetype></basename></menu></csa></tag></server_name></bootservicetype></bootservicetype></basename></server_name></server></bootservicetype></server_name></server></basename></csa></tftp_servername></server></servername></filename></tag></tftp_servername></tag></server></servername></tag></tag></tag></tag></tag></tag></tag></tag></tag></tag></ip></tag></tag></tag></tag></value></enterprise></option></tag></subscriber-id></tag></remote-id></tag></circuit-id></tag></mac></tag></user-class></tag></vendor-class></iana-enterprise></tag></interface></server></local></interface></interface></interface></server></server></server></local></server></local></local></path></path></value></value></opt></vendor-class></enterprise></opt></tag></tag></value></value></opt-name></opt></opt-name></opt></vendor-class></enterprise></opt></tag></tag></vendor-class></enterprise></opt></tag></option-name></option-name></path></path></lease_time></hostname></ipaddr></tag></client_id></hwaddr></lease></broadcast></netmask></mode></end-addr></start-addr></tag></tag></tag></mode></mode></mode></ipv6></ipv6></ipv4></ipv4></string></prefix></address></domain></interface></name></hex></rr-number></name></ttl></target></cname></replacement></regexp></service></flags></preference></order></name></target></name></text></text></name></ttl></ipv6-address></ipv4-address></name></name></weight></priority></port></target></domain></_prot></_service></hostname></preference></hostname></mx></ipset></ipset></domain></ipaddr></domain></ipaddr></domain></ipaddr></domain></port></interface></source-ip></port></ipaddr></prefix-len></ip-address></port></interface></source-ip></port></ipaddr></domain></port></interface></source-ip></port></ipaddr></domain></mask></new-ip></end-ip></start-ip></old-ip></domain></domain></domain></service-name></ipaddr></ip-address></interface></domain></port></port></size></lines></facility></time></time></time></time></time></time></path>
```