...
date: '2020-04-13T15:56:49+00:00'
status: publish
permalink: /2020/04/13/sql-server-%e5%88%9b%e5%bb%ba%e5%88%86%e5%8c%ba-%e5%88%a0%e9%99%a4%e5%88%86%e5%8c%ba-%e7%ad%89%e5%b8%b8%e7%94%a8%e5%88%86%e5%8c%ba%e8%af%ad%e5%8f%a5
author: 毛巳煜
excerpt: ''
type: post
id: 5315
...
-- （name:文件名，fliename:物理路径文件名，size:文件初始大小kb/mb/gb/tb，filegrowth:文件自动增量kb/mb/gb/tb/%,maxsize:文件可以增加到的最大大小kb/mb/gb/tb/unlimited）
...
**注：建立唯一索引（聚集或者非聚集）时，分区列必须出现在索引列中。**  
`解释：right，left用法`
...
WHERE t.name = 'DFG_dc_flowdata_deliver_sale_partition' AND i.type 
```
...
JOIN sys.indexes AS i   
    ON t.[object_id] = i.[object_id]   
    AND i.[type] = 1 -- because 0 = non-partitioning column   
...
select <span class="katex math inline">partition.myRangePF1('2015-04-04 ')

---查看指定分区中的数据记录
select * from dbo.DFG_dc_flowdata_deliver_inventory where</span>partition.myRangePF1(paas_update_time)=4
