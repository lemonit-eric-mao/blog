...
date: '2019-11-20T10:40:51+00:00'
status: publish
permalink: /2019/11/20/python-%e6%a0%b9%e6%8d%ae%e9%a1%b9%e7%9b%ae%e5%90%8d%e7%a7%b0%e7%94%9f%e6%88%90docker-compose-%e6%96%87%e4%bb%b6
author: 毛巳煜
excerpt: ''
type: post
id: 5137
...
```yml
<br></br><br></br>  <span class="katex math inline">{project_name}:
    container_name:</span>{project_name}
    image: sinoeyes.io/dev2/<span class="katex math inline">{image_name}:</span>{version}
    restart: always
    network_mode: host
    volumes:
      - ../logs/<span class="katex math inline">{project_name}:/var/log/apps
      - ../config/logback-spring.xml:/opt/logback-spring.xml
    environment:
      - JVM_OPTS=</span>{jvm_opts}
      - spring.cloud.config.uri=http://config-server:20001
      - spring.cloud.config.profile=prd2
      - user.timezone=GMT+08
      - mainClass=<span class="katex math inline">{main_class}
    ports:
      -</span>{port}:${port}
